...
do
  echo -e "\n\nProcessing $f file..."
  mv $f temp.json
  jq -r --arg dbhost "$POSTGRES_HOST" --arg dbport "$POSTGRES_PORT" --arg dbname "$POSTGRES_DB" --arg dbuser "$POSTGRES_USER" --arg dbpassword "$POSTGRES_PASSWORD" '.config["database.hostname"] |= $dbhost | .config["database.port"] |= $dbport | .config["database.dbname"] |= $dbname | .config["database.user"] |= $dbuser | .config["database.password"] |= $dbpassword' temp.json > $f
  rm temp.json
  curl -s -X POST -H "Accept:application/json" -H  "Content-Type:application/json" http://connect:8083/connectors/ -d @$f
done
